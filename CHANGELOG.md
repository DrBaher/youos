...
- **Per-digest `account`** — a digest can now target a specific account (e.g. only `drbaher@gmail.com`); empty = all configured accounts (prior behavior). The scheduler skips a digest whose `account` is set and ≠ the account it's running for, so a scoped digest can't fan out to other inboxes. Authorable via the `/digests` UI + API.
