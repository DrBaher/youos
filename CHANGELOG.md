...
## Unreleased — The Wire: comprehensive newsletter digest, native port (b285)

Brings the OpenClaw "The Wire" skill into YouOS as a first-class capability. Once a day (default **weekdays 19:00**) it fetches the day's newsletters across all configured accounts (work accounts contribute categorized promotions/updates only), asks the cloud model to extract **every** individual story, deduplicate across sources, and group them into themed sections (Top Stories → AI & Tech → … → Interesting Links → 🛍️ Promotions, with 💸 Fundraising & Deals always present), then delivers **one** Gmail-safe **HTML** email and archives the sources (except a never-archive allow-list). Each issue carries a sequential **edition** number, seeded from the prior OpenClaw tracker so the numbering continues unbroken.

New `app/agent/wire_digest.py` reuses the digest engine's safety machinery so the never-send invariant is unchanged: the send is gated on `agent.wire.enabled` **and** the shared `agent.send.enabled` frontier **and** the outbound kill-switch (all three open to send), and the same UNIQUE-index period claim (`name="Wire"`, `account="*"`) makes it at-most-once per day across overlapping ticks. The render is hardened against a half-baked issue going out — the locked HTML template is deterministic (the model fills only the section cards), output is rejected if it contains template placeholders or markdown artifacts, and a deterministic grouped-list fallback covers the model being unavailable. An empty day sends/archives/bumps nothing.

Wiring: scheduler runs it once per tick (not per account); `agent.wire.enabled` is whitelisted in feature flags and marked send-frontier-sensitive (a network token can't self-arm it); `youos wire run [--preview] [--days N]` / `youos wire status` CLI; and `gmail_write.send_email` gains a `body_html` path (gog `--body-html-file -` via stdin so a large digest avoids the argv limit). Cloud egress: newsletter bodies go to the frontier model (same as the original skill); newsletters are bulk mail, the digest is read-only, and it crosses no new outbound frontier beyond the gated send. Tunable under `agent.wire` (hour, weekdays_only, days_back, summary_model, skip/promo/archive-exclusion lists). 24 tests cover the gating, empty-day no-op, dedup, edition continuity/bump-only-on-send, the filters, the weekdays-only schedule, and the placeholder/markdown reject.
...
Follow-up to the b127 audit — adds tests for the error/edge branches the audit flagged as untested (no source changes; pure coverage hardening of the safety-critical paths):
...
Live-verified by running `youos triage --account you@gmail.com --window 3d --limit 8` after the fix — successfully fetched 8 messages, hard-skipped 6 (GitHub CI, Substack), surfaced 1 borderline, drafted 1 (a false positive worth dismissing as `noise`).
...
|---|---|---|
| **gog** (b47) | Live-verified | 2 real drafts created + deleted on you@gmail.com |
...
youos config set agent.accounts '["baher@work.example", "you@gmail.com"]'  # optional; falls back to user.emails
...
### Agent triage — further filter tuning from the 14-day work sample
The 14-day window on `baher@work.example` surfaced 5 false-positives, all transactional notifications that produced hallucinated drafts (wrong names, wrong topics — random Baher-corpus context plugged in). Three root causes, one fix PR each:
...
### Expected effect on the 14-day work sample
...
### Tests
Six in `test_cold_outreach`: the Jess case (positive), Alex pricing inquiry (true negative), Sam friend message (true negative), internal teammate quick-chat (true negative — guards against false positive on "quick chat" subjects from `@work.example` peers), high-confidence body-pattern double-weighting, and the `DECLINE_NUDGE` constant. 50/50 across the affected suites.
...
QA-driven content-quality fix #1: BaherOS drafts of short technical questions were returning **only the signature block** (`"Baher Al Hakim CEO / Work AI w: work.example e: …"`) with no actual answer. Diagnosis confirmed the LoRA was emitting only the signature half because every exemplar in its training + every exemplar in the prompt ended with `… Baher Al Hakim`. `strip_signature` removes the contact-detail block (`CEO / Work AI w: …`) but leaves the trailing user-name intact — it's not at line start, so the line-anchored patterns miss it.
...
Bumped `sender_type_boost` and `sender_domain_boost` from `0.15/0.10` to `0.20/0.20`, ran retrieval against the Alex/Stripe inbound (with `sender_type_hint`/`sender_domain_hint` populated as `generate_draft` does), and it **regressed**: same-domain boosting amplified Baher's own-account `@work.example` pairs (recurring meetings/check-ins) over the topic-relevant pricing exemplars. Reverted; left a comment explaining why the boost is the wrong lever for *topic* mismatch (intros vs pricing) — that's what semantic+candidate-pool fix. Boosts remain for the rarer "have-I-talked-to-X" axis.
...
- **Run-on inline signature.** The LoRA emits `"Cheers, Baher Al Hakim CEO / Work AI w: work.example e: baher@…"` *on a single line*, but the existing signature patterns were line-anchored (`^Cheers,$` with `MULTILINE`) and missed the inline form. Added three inline patterns to `_build_signature_patterns`: role+separator+capital (`CEO / Work`), single-letter contact marker followed by URL/email/phone (`w: work.example`), and "Sent from my <device>" inline. Specific enough to avoid eating legitimate prose.
...
- Vendor draft: `…I&#39;d love to share… Cheers, Baher Al Hakim CEO / Work AI w: work.example e: baher@` → **`…I'd love to share… Cheers, Baher Al Hakim`** (`stripped_trailing_signature`, `decoded_html_entities`)
- Edge case (no sender name): `…Thanks, Baher Al Hakim CEO / Work AI w:` → **`…Thanks, Baher Al Hakim`** (`stripped_trailing_signature`); the beta.20 `Hi,` greeting fix stays clean.
