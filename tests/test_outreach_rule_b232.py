...
_RULE = {
    "match": {"sender": "noreply@medicus.example", "subject_contains": "New demo request"},
...
def test_extract_lead_contact_never_returns_own_address():
    c = extract_lead_contact(
        "Form copy to baher@medicus.example. Lead: jane@corp.example",
        exclude_emails=["baher@medicus.example"],
    )
...
def test_match_outreach_rule_matches_and_filters_action():
    rules = [
        {"match": {"sender": "noreply@medicus.example"}, "action": "hold", "value": None},
        normalize_rule(_RULE),
    ]
    r = match_outreach_rule(
        rules, sender_email="noreply@medicus.example", domain="medicus.example",
...
def outreach_env(monkeypatch, tmp_path):
    ...
    msgs = [
        InboxMessage(
            ...
            sender="Website <noreply@medicus.example>",
            sender_email="noreply@medicus.example",
