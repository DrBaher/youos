...
def test_gog_creates_draft_and_extracts_id(monkeypatch):
    ...
    result = create_draft(
        account="me@work.example", thread_id="thr_42",
    ...
    assert "--account" in cmd and cmd[cmd.index("--account") + 1] == "me@work.example"
...
def test_gws_creates_draft_and_extracts_id(monkeypatch):
    ...
    assert "--user" in cmd and cmd[cmd.index("--user") + 1] == "me@work.example"
