...
def test_internal_domain_is_internal(monkeypatch):
    monkeypatch.setattr("app.core.sender.get_internal_domains", lambda: {"medicus.ai"})
    assert classify_sender("baher@medicus.ai") == "internal"
...
def test_internal_overrides_role_mailbox_but_automated_wins(monkeypatch):
    ...
    assert classify_sender("info@medicus.ai") == "internal"
    # but a hard automated local at an internal domain is still machine mail
    assert classify_sender("noreply@medicus.ai") == "automated"
