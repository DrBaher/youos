...
def regenerate(row_id: int, body: RegenerateBody, request: Request) -> dict:
    ...
    # If this was a SURFACED thread (no draft) and we just persisted one on
    # request, promote it to a draft so it can be pushed/sent like any other.
    # Gated on ``persisted`` (mark_amended succeeded) so a dismissed/sent row —
    # where mark_amended no-ops — can't be flipped to tier='draft' in a stale,
    # draftless state.
    if persisted and row.get("tier") == "surface" and store.get(db_url, row_id).get("status") in ("pending", "amended"):
